...
## Temperatura
res_temp_weatherlink <- tbl(con, schema_weatherlink_sensor_772005) |>
...
res_temp_plugfield <- tbl(con, schema_plugfield) |>
  filter(sensor == 8) |>
  filter(time >= time_uct) |>
  select(time, value) |>
  collect() |>
...
## Umidade
res_umid_weatherlink <- tbl(con, schema_weatherlink_sensor_772005) |>
...
res_umid_plugfield <- tbl(con, schema_plugfield) |>
  filter(sensor == 11) |>
  filter(time >= time_uct) |>
  select(time, value) |>
  collect() |>
...
## Pressão
res_press_weatherlink <- tbl(con, schema_weatherlink_sensor_772003) |>
...
res_press_plugfield <- tbl(con, schema_plugfield) |>
  filter(sensor == 23) |>
  filter(time >= time_uct) |>
  select(time, value) |>
  collect() |>
...
## Chuva
res_chuva_weatherlink <- tbl(con, schema_weatherlink_sensor_772005) |>
...
res_chuva_plugfield <- tbl(con, schema_plugfield) |>
  filter(sensor == 35) |>
  filter(time >= time_uct) |>
  select(time, value) |>
  collect() |>
...
## Vento
res_vento_weatherlink <- tbl(con, schema_weatherlink_sensor_772005) |>
...
res_vento_plugfield <- tbl(con, schema_plugfield) |>
  filter(sensor == 36) |>
  filter(time >= time_uct) |>
  select(time, value) |>
  collect() |>
